...
public class PhoneInfo {
	...
	public static String getDeviceInfo(){
		...
		buffer.append("PhoneModel=").append(Build.MODEL).append("\n");
		buffer.append("AndroidVersion=").append(Build.VERSION.RELEASE).append("\n");
		//buffer.append("Product=").append(android.os.Build.PRODUCT).append("\n");
		buffer.append("Device=").append(Build.DEVICE).append("\n");
		buffer.append("Display=").append(Build.DISPLAY).append("\n");
		buffer.append("BuildID=").append(Build.ID).append("\n");
...
//		String operatorName = null;
        ...
        try {
            TelephonyManager manager = (TelephonyManager) ComponentContext.getContext().getSystemService(Context.TELEPHONY_SERVICE);
//            phonetype = manager.getPhoneType();
//            operator = manager.getNetworkOperator() + "-" + manager.getSimOperator();
//            operatorName = manager.getNetworkOperatorName() + "-" + manager.getSimOperatorName();
            country = manager.getSimCountryIso(); // SIM卡对应的国家标识
            language = RegionUtil.getLocalString();
        } catch (Throwable tr) {
        }
...
//						|| "GT-I9502".equals(Build.MODEL)
	...
	public static boolean isNotSupportVideoRender() {
		if ("GT-I9300".equals(Build.MODEL)
				...
				|| ("SM-N910F".equals(Build.MODEL) && Build.VERSION.SDK_INT != Build.VERSION_CODES.LOLLIPOP)) {
			return true;
		}
		return false;
	}
