...
public class GPUImageFilterGroup extends GPUImageFilter {
    ...
    public void addFilter(List<GPUImageFilter> filters) {
        if (filters == null) {
            return;
        }
    ...
    public void removeFilter(List<GPUImageFilter> filters) {
        if (filters == null || filters.size() == 0) {
            return;
        }
        mFilters.removeAll(filters);
        updateMergedFilters();
    }
    ...
    public void updateTimestamp(long timestamp) {
        ...
        if (mMergedFilters != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (filter instanceof GPUImageDynamicFilter) {
                    ((GPUImageDynamicFilter) filter).updateTimestamp(mTimestamp);
                }
            }
        }
    }
        ...
        if (mMergedFilters != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (isNotLast) {
                    GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, 0);
                    // 线上这里有报空指针错误
                    if (mFrameBufferTextures != null && i < mFrameBufferTextures.length) {
