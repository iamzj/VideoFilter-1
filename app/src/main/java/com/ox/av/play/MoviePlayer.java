...
public class MoviePlayer {
    ...
    private void play() throws IOException {
        new Thread(mVideoTask, "Movie Player Video").start();
        new Thread(mAudioTask, "Movie Player Audio").start();
    }
    ...
    private Runnable mAudioTask = new Runnable() {
        @Override
        public void run() {
            try {
                playAudio();
    ...
    private void playVideo() throws IOException {
        ...
        try {
            ...
            videoDecoder.configure(format, mOutputSurface, null, 0);
            videoDecoder.start();

            doExtract(extractor, trackIndex, videoDecoder, mFrameCallback);
        } finally {
    ...
    private void doExtract(MediaExtractor extractor, int trackIndex, MediaCodec decoder,
        ...
        /*int sz = decoderInputBuffers[0].capacity();
        if (sz <= 0)
        ...
        while (!outputDone) {
            ...
            if (!outputDone) {
                int decoderStatus = decoder.dequeueOutputBuffer(mVideoBufferInfo, TIMEOUT_USEC);
                if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    boolean doRender = (mVideoBufferInfo.size != 0);
                    ...
                    if (doRender && frameCallback != null) {
                        frameCallback.preRender(mVideoBufferInfo.presentationTimeUs);
                    }
    ...
    private void playAudio() throws IOException {
        ...
        int audioInputBufferSize = 0;
        AudioTrack audioTrack = null;
        byte[] mAudioOutTempBuf = null;
            ...
            while (!outputDone) {
                ...
                if (!inputDone) {
                    ...
                    if (inputBufIndex >= 0) {
                        ...
                        if (chunkSize < 0) {
                            // End of stream -- send empty frame with EOS flag set.
                            audioDecoder.queueInputBuffer(inputBufIndex, 0, 0, 0L,
                                    MediaCodec.BUFFER_FLAG_END_OF_STREAM);
                            inputDone = true;
                            if (VERBOSE) Log.d(TAG, "sent input EOS");
                        } else {
                            long presentationTimeUs = extractor.getSampleTime();
                            audioDecoder.queueInputBuffer(inputBufIndex, 0, chunkSize,
                                    presentationTimeUs, 0 /*flags*/);
                            if (VERBOSE) {
                                Log.d(TAG, "submitted frame " + inputChunk + " to dec, size=" +
                                        chunkSize);
                ...
                if (!outputDone) {
                    int decoderStatus = audioDecoder.dequeueOutputBuffer(mAudioBufferInfo, TIMEOUT_USEC);
                    if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                        // no output available yet
                        if (VERBOSE) Log.d(TAG, "no output from decoder available");
                    } else if (decoderStatus == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                        ...
                        ByteBuffer outputBuffer = mAudioOutputBuffers[decoderStatus];
                        if (mAudioBufferInfo.size > 0) {
                            if (mAudioOutTempBuf.length < mAudioBufferInfo.size) {
                                mAudioOutTempBuf = new byte[mAudioBufferInfo.size];
                            }
                            outputBuffer.position(0);
                            outputBuffer.get(mAudioOutTempBuf, 0, mAudioBufferInfo.size);
                            outputBuffer.clear();
                            if (audioTrack != null)
                                audioTrack.write(mAudioOutTempBuf, 0, mAudioBufferInfo.size);
                        }
                        ...
                        if (doLoop) {
                            ...
                            extractor.seekTo(0, MediaExtractor.SEEK_TO_CLOSEST_SYNC);
                            inputDone = false;
                            audioDecoder.flush();    // reset decoder state
                        }
                    }
                }
            }
    ...
    public static class PlayTask {
        ...
        public synchronized void requestStart() {
            synchronized (mPlayer.mStopRequestLock) {
                if (mPlayer.mIsStopRequested) {
                    mPlayer.mIsStopRequested = false;
                }
            }
            synchronized (mStopLock) {
                if (mStopped) {
